...
from algorithm import apply_target_calories, calculate_all
...
class CalorieCalculator(QMainWindow):
    ...
    def _build_consolidated_inputs(self) -> None:
        ...
        self.rb_lose = QRadioButton("Сброс веса (-15%)")
        self.rb_maintain = QRadioButton("Поддержание веса")
        self.rb_gain = QRadioButton("Набор массы (+10%)")
    ...
    def _enforce_age_restrictions(self) -> None:
        ...
        self._update_goal_labels(under_18)
        self._last_under_18 = under_18

    def _update_goal_labels(self, under_18: bool) -> None:
        deficit_pct = "10%" if under_18 else "15%"
        self.rb_lose.setText(f"Сброс веса (-{deficit_pct})")
        self.rb_gain.setText("Набор массы (+10%)")
    ...
    def _ask_apply_min_calorie(self, calculated: float, min_cal: int) -> bool:
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle("Внимание!")
        msg.setText(
            f"Расчётная калорийность ({fmt_int(calculated)} ккал) ниже безопасного "
            f"медицинского минимума."
        )
        msg.setInformativeText(
            f"Рекомендуется поднять её до {min_cal} ккал. Применить безопасный порог?"
        )
        apply_btn = msg.addButton("Применить безопасный порог", QMessageBox.AcceptRole)
        keep_btn = msg.addButton("Оставить расчётное значение", QMessageBox.RejectRole)
        msg.setDefaultButton(apply_btn)
        msg.exec_()
        return msg.clickedButton() == apply_btn
        ...
        try:
            ...
            if age < 18 and self._get_goal_code() != "maintain":
                QMessageBox.warning(
                    self,
                    "Предупреждение WHO",
                    "Для лиц младше 18 лет не рекомендуется агрессивный дефицит или профицит калорий.\n"
                    "Дефицит/Профицит будет автоматически ограничен безопасными рамками (не более 10%).\n"
                    "Настоятельно рекомендуется проконсультироваться со специалистом."
                )
            ...
            if results.get("below_min"):
                min_cal = results["min_cal"]
                if self._ask_apply_min_calorie(results["target_calories"], min_cal):
                    results = apply_target_calories(
                        results, float(min_cal), weight, goal, gender
                    )
    ...
    def reset_fields(self) -> None:
        ...
        self.rb_maintain.setChecked(True)
        self.rb_mifflin.setChecked(True)
        self._update_goal_labels(False)
        self.activity_combo.setCurrentIndex(2)
